		...
		, m_iconSize( 22 )
{
	...
	/* initView updates the main widget, system tray icon and the ASUS led if applicable
	   it needs to be called before initActions() because that uses some icons that get
...
void KWiFiManagerApp::slotChangeWindowCaption()
{
	...
	{
		this->setCaption( i18n( "Interface %1" ).arg( device->get_interface_name() ) );
	}
}

KWiFiManagerApp::~KWiFiManagerApp()
{
	kdDebug() << "Thank you for using KWiFiManager. You have made a simple program very happy." << endl;
	delete statistik;
	delete tricorder_trigger;
}

void KWiFiManagerApp::init_whois_db()
{
	...
	if ( !locations->exists() )
	{
		kdDebug() << "A grave error occurred while searching for files with AccessPoint information." << endl;
	}
...
void KWiFiManagerApp::slotLogESSID( QString essid )
{
...
void KWiFiManagerApp::slotTXPowerChanged()
{
	fileDisableRadio->setChecked( device->get_txpower_disabled() );
}

void KWiFiManagerApp::slotChangeTrayIcon()
{
	QPixmap* ICON;
	int sig, noi, str;
	double freq = 0.;
	...
	if ( settingsUseAlternateCalc->isChecked() )
		str = sig - noi;
	...
	if ( showStrength )
	{
		static QFont labelfont( "Monospace", 8 );
...
void KWiFiManagerApp::initActions()
{
	KConfig* config = kapp->config();
	...
	fileDisableRadio = new KToggleAction( i18n( "&Disable Radio" ), 0, this, SLOT( slotDisableRadio() ), actionCollection(), "disable_radio" );
	fileDisableRadio->setChecked( false );

	settingsUseAlternateCalc = new KToggleAction( i18n( "&Use Alternate Strength Calculation" ), 0, this, SLOT( slotToggleStrengthCalc() ), actionCollection(), "use_alt_calculation" );
	...
	settingsShowStatsNoise = new KToggleAction( i18n( "Show &Noise Graph in Statistics Window" ), 0, this, SLOT( slotShowStatsNoise() ), actionCollection(), "show_stats_noise" );

	settingsShowStatsNoise->setChecked( config->readBoolEntry( "showStatsNoise" ) );
	slotShowStatsNoise();  //set to value saved by KConfig

	settingsShowStrengthNumber = new KToggleAction( i18n( "&Show Strength Number in System Tray" ), 0, this, SLOT( slotToggleShowStrengthNumber() ), actionCollection(), "show_strength_number_in_tray" );
	...
	new KAction( i18n( "Configuration &Editor..." ), 0, this, SLOT( slotStartConfigEditor() ), actionCollection(), "configuration_editor" );
	new KAction( i18n( "Connection &Statistics" ), 0, this, SLOT( slotStartStatViewer() ), actionCollection(), "connection_statistics" );
	settingsAcousticScanning = new KToggleAction( i18n( "&Acoustic Scanning" ), 0, this, SLOT( slotToggleTric() ), actionCollection(), "acoustic_scanning" );
	settingsAcousticScanning->setChecked( config->readBoolEntry( "acousticScanning" ) );
#ifdef WITHOUT_ARTS
	settingsAcousticScanning->setEnabled( false );
#endif
	settingsStayInSystrayOnClose = new KToggleAction( i18n( "Stay in System &Tray on Close" ), 0, this, SLOT( slotToggleStayInSystray() ), actionCollection(),"stay_in_systray_on_close" );
	settingsStayInSystrayOnClose->setChecked( config->readBoolEntry( "stayInSystrayOnClose" ) );
	createGUI();
}

void KWiFiManagerApp::initView()
{
	led = new Led();
