...
QTable* Interface_wireless_wirelessextensions::get_available_networks()
{
	...
	*iwlist << iwlist_bin << " " << interface_name << " " << "scan";

//    connect ( iwlist, SIGNAL ( readReady ( KProcIO * ) ), this, SLOT ( parseScanData ( KProcIO * ) ) );

	if ( !iwlist->start( KProcess::Block ) )
		KMessageBox::sorry( 0, i18n( "Unable to perform the scan. Please make sure the executable \"iwlist\" is in your $PATH." ), i18n( "Scanning not possible" ) );
	...
	for ( int i = 0; i<4; i++ ) {
		networks->adjustColumn( i );
	}

	return networks;
}

void Interface_wireless_wirelessextensions::parseScanData( KProcIO * iwlist )
{
	QString data;
