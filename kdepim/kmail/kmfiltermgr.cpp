...
KMFilterMgr::KMFilterMgr( bool popFilter )
    ...
    connect( kmkernel, SIGNAL( folderRemoved( KMFolder* ) ), this, SLOT( slotFolderRemoved( KMFolder* ) ) );
}
...
void KMFilterMgr::clear() {
    mDirtyBufferedFolderTarget = true;
    for (QValueListIterator<KMFilter*> it = mFilters.begin(); it != mFilters.end(); ++it) {
        delete *it;
    }
}
...
int KMFilterMgr::processPop( KMMessage * msg ) const {
    for ( QValueListConstIterator<KMFilter*> it = mFilters.constBegin(); it != mFilters.constEnd(); ++it ) {
